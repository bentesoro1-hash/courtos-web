          ...
          {PREVIEWS.map((preview) => (
            ...
            >
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', background: 'linear-gradient(to bottom, rgba(12,12,12,0.1) 0%, rgba(12,12,12,0.5) 40%, rgba(12,12,12,0.95) 100%)', padding: 28 }}>
                <div style={{ background: 'rgba(61,190,107,0.1)', border: '1px solid rgba(61,190,107,0.3)', borderRadius: 8, padding: '5px 12px', marginBottom: 12 }}>
                  <span style={{ color: '#3DBE6B', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em' }}>BETA ACCESS ONLY</span>
                </div>
                <h3 style={{ color: '#F0F0F0', fontSize: 18, fontWeight: 700, textAlign: 'center', marginBottom: 6 }}>{preview.title}</h3>
                <p style={{ color: '#888', fontSize: 13, textAlign: 'center', marginBottom: 20, lineHeight: 1.5 }}>{preview.desc}</p>
                <a href="#beta" style={{ background: '#3DBE6B', color: '#000', padding: '8px 20px', borderRadius: 8, fontSize: 12, fontWeight: 700, textTransform: 'uppercase', textDecoration: 'none', letterSpacing: '0.05em' }}>Unlock Access</a>
              </div>
            </div>
          ))}
        </div>
